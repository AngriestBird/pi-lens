...
**Inline-blocker self-drift baseline (#2982 round 4).** `PipelineResult` carries the size and sha256 for the file content already read by `runPipeline` beside the inline-blocker evidence. `recordInlineBlockers` stamps that baseline in the same synchronous write, so no delayed second read can attach evidence to a newer verdict. Missing baselines and expired self-drift bounds use the bounded `self-drift-unverifiable` ledger kind with distinct file subjects; aggregate hash-budget exhaustion remains sweep metadata. Same-size edits demote, identical bytes after an mtime-only change remain authoritative, and deferred mutation remains a safe demotion.
