...
7. **A vacuous test fixture that never exercises the code under test.** A hardcoded version literal orphaned by a version bump; a mock missing the property the guarded code reads (so both guard branches pass for free); a drive-letter literal fed to a normalizer as an *expected key* on the assumption it's a no-op; or a suite-wide environment default disabling the mechanism whose side effects the test counts. *Screen:* every regression test must FAIL on pre-fix code, and the fixture must actually reach an armed code path. Side-effect-count tests opt back in with save/restore or assert a positive control. *e.g.* #1114 (kill-process-tree mock had no `.once`/`.killed`, so the SIGKILL-escalation guard passed vacuously — the escalation was dead code), #1089/#1106 (fixture version drift), #1139/#1150 (Windows-shaped literal as expected key — see the OS-agnostic paragraph), #1759 (17 tests asserted a suite-disabled no-op path). *Detect:* the "confirm the regression test fails against pre-fix code" step; a mock asserted on a method it never defines; an environment gate is off for the whole suite. Not ast-grep-able (#1759). *Prevention:* this entry is the review-time catch. The authoring-time form is the six **Test-authoring screens** in the "Test requirements" section below (#1829): parallel path, invisible skip, wrong-layer pin, ambient-inspection double, env leakage, and loose bound. Each is answerable while you write the test.
	**Inverse variant — a fixture that pins a shape the producer cannot emit.** The mock is green and the guard runs, but on a result no real run yields, so the guard's REAL input never reaches it. Four separate truncation tests paired `outputTruncated: true` with a clean exit status; a cap kill is a SIGTERM and always arrives with `failure: "signal"`, so all four passed while the production guards sat behind checks that swallowed the real shape (#2100). *Screen:* when a fixture claims a producer's output, derive it from that producer — a shared builder anchored to one live-run test, not a hand-written literal per test file.
...
**A truncation guard is read BEFORE the failure/status handling (#2100).** `safeSpawnAsync` only sets `outputTruncated` when the call site passes `maxOutputBytes`, and reaching that cap makes `stopForOutputLimit` kill the tree — so the result a real cap produces is `status: null` + `failure: "signal"` + `spawnFailure.kind === "killed"` + `outputTruncated: true`, not the clean-status pairing hand-written mocks keep inventing. A guard placed after a `spawnFailure`/`failure`/`status` check therefore describes only the rarer race where the tool exits before the SIGTERM lands, and every cap kill is reported as a kill, a server error, or a git failure instead of as the truncation that caused it. Three screens when adding either half. (1) A `maxOutputBytes` needs a comment naming why that value against the tool's real output scale (`git-tracked-ignore.ts`'s `MAX_LS_FILES_OUTPUT_BYTES`, `helm-render.ts`'s `MAX_REPORT_BYTES`), pinned by a test asserting `expect.objectContaining({ maxOutputBytes: N })` on the spawn options. (2) NEVER read `outputTruncated` directly — use `truncatedByOutputCap` (`clients/spawn-output-cap.ts`), and read it FIRST in its function, ahead of everything else. The flag alone is not the cap's verdict: it is spread into EVERY resolve branch and `timedOut`/`aborted` are set unconditionally, so a noisy tool that also hangs or gets interrupted carries it under a `timeout`/`aborted` failure that owns its own classification — reading the bare flag first turns every such run into a bogus truncation report. The predicate lives in its own dependency-free module because `spawn-outcome.ts` is on the shared runner path that dozens of test files reach with a bare `vi.mock("safe-spawn.js")`; importing a value from safe-spawn there would force every one of those mocks to re-export it. (3) Prove the truncation path against `capKilledSpawnResult` AND the timeout/abort variants (`tests/support/spawn-shapes.ts`, all anchored to live spawns in `tests/clients/safe-spawn-ambient-signal.test.ts`) — a guard that is right about the cap kill and wrong about the timeout is a net regression. Reordering also does not license SKIPPING work the tool completed: `helm-render` only treats a truncated render as UNCHECKED when helm never reported an exit status, because a helm that exited 0 finished writing `--output-dir` and every manifest is still validatable from disk. On the ledger side, `classifyRunOutcome` carries the cap as `outputCapped` evidence that changes the `formatToolFailure` WORDING only ("was stopped at its output cap" rather than "was killed by SIGTERM") and never the `kind` — the shared seam must not be re-classified for one tool's benefit.
