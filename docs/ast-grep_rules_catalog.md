...
| `no-bare-object-param` | error | pi-lens | Parameter typed bare 'object' — guarantees nothing about shape, use Record<string, unknown> or a real interface |
| `no-case-declarations` | error | pi-lens | Unexpected lexical declaration in case block — wrap in braces to restrict scope |
| `no-chained-type-assertions` | error | pi-lens | Chained type assertion (x as A as B) — stacking assertions with no runtime check between them |
...
| `no-known-value-widening` | hint | pi-lens | const initialized from an object literal, widened by a Record<...> annotation — prefer inference or 'satisfies' |
...
| `no-shape-in-symbol-names` | hint | pi-lens | Symbol name contains 'shape' — name it for its domain role, not its structure |
