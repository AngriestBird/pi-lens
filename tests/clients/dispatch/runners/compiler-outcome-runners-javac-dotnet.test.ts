...
vi.mock("../../../../clients/tool-probe.js", () => ({
	probeToolAsync: vi.fn(async () => ({ error: new Error("unavailable"), status: null })),
}));
vi.mock(
...
async function dispatchOutcome(
	...
	try {
		...
		safeSpawnAsync.mockResolvedValue(
			available
				? typeof result === "function"
					? result(filePath)
					: result
				: { error: new Error("unavailable"), status: null, stdout: "", stderr: "" },
		);
		const { createDispatchContext, dispatchForFile, RunnerRegistry } =
			await import("../../../../clients/dispatch/dispatcher.js");
		const runner = (await import(`../../../../clients/dispatch/runners/${tool}.js`)).default;
		const { getDegradationSummary, resetDegradationLedger } =
			await import("../../../../clients/degradation-ledger.js");
		resetDegradationLedger();
		const registry = new RunnerRegistry();
		registry.register(runner);
		let observed: { status?: string; semantic?: string; diagnostics: unknown[] } = {
			diagnostics: [],
		};
		const output = await dispatchForFile(
			...
			registry,
			(_id, result) => {
				observed = { status: result.status, semantic: result.semantic, diagnostics: result.diagnostics };
			},
		);
		return { ...observed, output: output.output, ledger: getDegradationSummary() };
	} finally {
		env.cleanup();
	}
}
	...
	try {
		...
		for (const filePath of files)
			await dispatchForFile(
				createDispatchContext(filePath, env.tmpDir, { getFlag: () => false }, new FactStore()),
				[{ mode: "all", runnerIds: [tool] }],
				registry,
			);
...
describe("compiler runner outcome seam (#1816)", () => {
	...
	for (const tool of ["javac", "dotnet-build"] as const) {
		it.each([
			...
			["status 0 stderr noise", { status: 0, stdout: "", stderr: "compiler banner" }],
		])(`${tool}: %s stays clean through dispatch`, async (_name, result) => {
			const observed = await dispatchOutcome(tool, "clean", { error: null, ...result });
		...
		it(`${tool}: preserves nonzero valid findings through dispatch`, async () => {
			const observed = await dispatchOutcome(tool, "findings", (filePath) => ({
				error: null, status: 1, stdout: "", stderr: findings(tool, filePath),
			}));
		...
		it.each([
			...
			const observed = await dispatchOutcome(tool, "parse-error", {
				error: null, status: 1, ...streams,
			});
			expect(observed.status).toBe("failed");
			expect(observed.diagnostics[0]).toMatchObject({ id: `${tool}:parse-error:1` });
		...
		it(`${tool}: signal termination is not succeeded`, async () => {
			const observed = await dispatchOutcome(tool, "signal", {
				error: null, status: null, signal: "SIGTERM", stdout: "", stderr: "",
			});
			expect(observed.status).toBe("skipped");
		...
		it(`${tool}: rejected invocation is a parse error`, async () => {
			const observed = await dispatchOutcome(tool, "rejected", {
				error: null, status: 2, stdout: "", stderr: "unknown option",
			});
			expect(observed.status).toBe("failed");
			expect(observed.diagnostics[0]).toMatchObject({ id: `${tool}:parse-error:1` });
		});
		...
		it(`${tool}: renders the shared witness text`, async () => {
			...
			const golden = fs.readFileSync(
				path.resolve("tests/fixtures/witness/runner-outcome-javac-dotnet-build", `${tool}-rendered.txt`),
				"utf8",
			);
			expect(formatDiagnostics(observed.diagnostics as never[], "blocking")).toBe(golden);
		});

		it(`${tool}: bounds empty-result to one ledger row across two files`, async () => {
			const ledger = await dispatchTwoEmptyFiles(tool);
			expect(ledger).toHaveLength(1);
			expect(ledger[0]).toMatchObject({ kind: "runner-empty-result", count: 2 });
		});
	}
});
