...
async function dispatchOutcome(tool: Tool, result: SpawnResult) {
	...
	try {
		...
		fs.writeFileSync(filePath, tool === "actionlint" ? "name: CI\n" : "defmodule Example do\nend\n");
		if (tool === "credo") fs.writeFileSync(path.join(env.tmpDir, "mix.exs"), "defmodule MixProject do\nend\n");
...
const findings = {
	actionlint: JSON.stringify([{ message: "bad workflow", line: 2, column: 3, kind: "syntax" }]),
	credo: JSON.stringify({
		issues: [{ filename: "lib/example.ex", line_no: 2, column: 1, message: "bad credo", category: "Style", check: "Credo.Check", priority: 5 }],
	}),
};
...
describe("actionlint and credo shared runner outcome seam (#1816)", () => {
	...
	for (const tool of ["actionlint", "credo"] as const) {
		it.each([
			...
			["signal", { status: null, stdout: "", stderr: "", signal: "SIGTERM" as const }, "skipped"],
			["rejected", { status: 2, stdout: "", stderr: "unknown option" }, "failed"],
			...
			if (expected === "skipped") expect(observed.ledger[0]?.latestReasons[0]?.subject).toBe(tool);
		});

		it(`${tool}: unavailable does not report clean`, async () => {
			actionlintAvailable = false;
			const observed = await dispatchOutcome(tool, { error: new Error("missing"), status: null, stdout: "", stderr: "" });
			expect(observed.status).toBe("skipped");
		});
	}

	it("renders independent witness goldens for both finding parsers", async () => {
		for (const tool of ["actionlint", "credo"] as const) {
			const observed = await dispatchOutcome(tool, { error: null, status: 1, stdout: findings[tool], stderr: "" });
			expect(observed.output.trim()).toBe(
				fs.readFileSync(
					path.resolve("tests/fixtures/witness/runner-outcome-actionlint-credo", `${tool}.txt`),
					"utf8",
				).trim(),
			);
		}
