 ...
 *      answers instantly with a fake token. Wait out the real cooldown.
 *   4. Trigger a third call -> assert a GENUINE re-probe happened (the fixture
 *      token is returned, not the cached `undefined`) and the decision log
 *      shows a fresh `available`/`success` record with no new degradation.
 *
...
async function main() {
	...
	try {
		...
		console.log("\nphase 3: restore the fixture + wait out the real cooldown");
		removeFixtureGh(binDir);
		writeFixtureGh(binDir, "ok");
		// TRANSIENT_BASE_COOLDOWN_MS is 30s; phase 1's probe already burned ~5s
		// of it waiting for the timeout, so wait the remainder plus a margin.
		await sleep(32_000);

		console.log("phase 4: trigger a genuine re-probe past the cooldown");
		const t3 = await resolveZizmorGitHubToken();
		assertEq(
		...
		group = summary.find((g) => g.kind === "mode-suppression");
		assertTrue(
			"phase4 no NEW degradation recorded on recovery",
			!!group && group.count === 2,
			`degradation count after recovery: ${group?.count ?? "no group"} (expected still 2)`,
		);
	} finally {
